...
import cqseur.dailyrewards.RewardSelector
...
class RewardScreen(private val offer: RewardOffer) : Screen(Text.literal("Daily Reward")) {
    ...
    private var autoClaimStarted: Boolean = false
    ...
    private fun maybeAutoClaim() {
        if (autoClaimStarted || selectedIndex != null || offer.id == "debug") return
        if (!ConfigManager.config.autoClaim) return

        val index = if (ConfigManager.config.autoClaimWeightsEnabled) {
            RewardSelector.bestIndex(offer)
        } else {
            0
        }
        if (index !in offer.cards.indices) return

        revealed[index] = true
        flipProgress[index] = 1f
        autoClaimStarted = true
        claimReward(index)
    }

    private fun claimReward(idx: Int) {
